...
describe('QuadraticEquation', () => {
  ...
  test('should get exception if some argument is not a number', () => {
    expect(() => equation.solve('a', 1, 1)).toThrow();
    expect(() => equation.solve(1, 'b', 1)).toThrow();
    expect(() => equation.solve(1, 1, 'c')).toThrow();
  });

  test('should get exception if arguments value is Infinity', () => {
    expect(() => equation.solve(Infinity, 1, 1)).toThrow();
    expect(() => equation.solve(1, Infinity, 1)).toThrow();
    expect(() => equation.solve(1, 1, -Infinity)).toThrow();
  });
});
